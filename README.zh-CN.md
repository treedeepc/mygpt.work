...
| Session 会不会长期保存 | 浏览器内先加密，临时会话密文与封装数据密钥最长保留 15 分钟 |
...
- 付款确认且账号授权有效后，通常 5～10 分钟完成，最迟不超过 1 小时；充值失败且未完成交付的，确认失败后 24 小时内原路发起全额退款。
- 付款状态与充值状态分开记录，过程更清楚。
...
- 下单表单从官方会话接口 JSON 读取 `sessionToken` 与 `user.email`，不要求浏览器 Profile 或浏览器存储；解密后只有规范化 Token 进入临时保险库。
- 登录会话在浏览器内加密后提交，只用于当前订单的账号核验和订阅操作。
- 临时会话密文与封装后的每单数据密钥最长保留 15 分钟；服务主密钥属于保留时间更长的受限运行配置，不在该期限内。
- 每次执行启动新的浏览器进程与 Context，持久队列只传订单标识；不声称每单容器隔离。
